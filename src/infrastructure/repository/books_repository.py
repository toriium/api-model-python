...
from src.infrastructure.dtos.tbl_books_dto import BookDataDTO
...
class BooksRepository:
    @staticmethod
    def find_book_by_id(book_id: int) -> tuple[Union[BookDataDTO, None], Union[SQLError, None]]:
        query_result = select_first_obj(obj_table=TblBooks, filter_by={"id": book_id})
        if query_result:
            return BookDataDTO.from_orm(query_result), None
        else:
            return None, None

    @staticmethod
    def find_book_by_name(name: int) -> tuple[Union[BookDataDTO, None], Union[SQLError, None]]:
    ...
    @staticmethod
    def insert_book(book: Book) -> tuple[Union[BookDataDTO, None], Union[SQLError, None]]:
        new_book = TblBooks()
