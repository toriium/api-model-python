...
async def create_token(form_data: OAuth2PasswordRequestForm = Depends()):
	...
	response_data = TokenOutput(
		access_token=access_token, token_type="bearer", expires_in=jwt_data.exp, refresh_token=refresh_token
	)
...
async def refresh_access_token(form_data: RefreshTokenForm):
	...
	response_data = TokenOutput(
		access_token=new_access_token, token_type="bearer", expires_in=jwt_data.exp, refresh_token=refresh_token
	)
